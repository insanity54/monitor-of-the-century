...
var handler = require(path.join(__dirname, 'handler'));
...
var add = function add(task, cb) {
    data.getTaskByName(task, function(err, t) {
        ...
        // j = cron.scheduleJob(task, t.schedule, function() {
        //     // do this at the scheduled time
        //     check.execute(task, handler.handle);
            
        //     check.execute(task, function(err, result) {
        //         if (!result.task) result['task'] = task;
        //         return handler.handle(err, result);
        ...
        //         // handle code
        //         // handle message
        //         //   handler needs two things
        //         //     the return content from check.execute
        //         //     the task that called the handler
        //         // return
        //     });
        // });
        ...
        j = cron.scheduleJob(task, t.schedule, function() {
            ...
            task.execute(taskName, handler.handle);
            
            // handler must:
            //   handle error
            //   handle code
            //   handle message
            //     handler needs three things
            //       the return code from check.execute
            //       [the return message from check.execute]
            //       the taskName of the task that called task.execute
            //   return
        });
        
        // save job to the datastore under the task.
        // if a task is deleted from tasks-enabled // @todo
        //     * stop job
